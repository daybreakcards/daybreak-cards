...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!idResp.ok) {
      ...
      console.error("Anthropic API error:", idResp.status, errText);
      return { statusCode: 502, headers: cors, body: JSON.stringify({ error: `Identification service error (${idResp.status})`, detail: errText }) };
    }
